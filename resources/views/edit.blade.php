        ...
        <form method="POST" action="/content/{{ $content->id }}">
          ...
          <div class="mb-4">
            ...
            <input name="title" id="title" cols="30" class="bg-gray-100 border-2 w-full
                          p-4 rounded-lg @error('content') border-red-500 @enderror"
              value="{{ $content->title }}"></input>
            @error('content')
          ...
          <div class="mb-4">
            ...
            <textarea name="content" id="content" cols="30" rows="4" class="bg-gray-100 border-2 w-full
                          p-4 rounded-lg @error('content') border-red-500 @enderror">{{ $content->content }}</textarea>
            @error('content')
              <div class="text-red-500 mt-2 text-2m">
                {{ $message }}
              </div>
          ...
          <div>
            ...
            <form method="POST" action="/content/{{ $content->id }}">
              ...
              <button type="submit"
                class="bg-red-500 text-white px-4 py-2 rounded font-medium inline-block">Delete</button>
            </form>
          </div>
        </form>
      </div>
